...
public partial class Grenade : Node2D
{
	...
	private void DoSteps()
	{
		...
		{
			var newPosition = Position + (Vector2.Down * Mathf.Sign(velocity.Y) * Mathf.Min(Mathf.Abs(velocity.Y), 1.0f));
		...
		{
			var newPosition = Position + (Vector2.Right * Mathf.Sign(velocity.X) * Mathf.Min(Mathf.Abs(velocity.X), 1.0f));
			var normal = GetNode<Map>("../Map").CollisionNormal(newPosition);
